...
const Header = ({ isSearchOpen, setIsSearchOpen }: HeaderProps) => {
  ...
  const { data, isLoading, error } = useQuery<Partial<Category>[]>({
    queryKey: ['categories'],
    queryFn: async () => {
        ...
        <div className="flex items-center justify-between h-16">
            ...
            <a href="/" onClick={handleNavigation("/")} className="flex items-center relative px-3 py-2">
              <div className="relative z-10 animate-float">
                <span className="font-heading text-2xl font-bold">
                  <span className="text-primary dark:text-accent">{siteConfig.name.split(' ')[0]}</span>
                  <span className="text-accent dark:text-primary">{siteConfig.name.split(' ')[1]}</span>
                </span>
              </div>
          ...
          <nav className="hidden md:block">
            <ul className="flex space-x-8">
              ...
              <li><a href="/about" onClick={handleNavigation("/about")} className="font-medium hover:text-primary dark:hover:text-accent transition-colors">About</a></li>
              <li><a href="/services" onClick={handleNavigation("/services")} className="font-medium hover:text-primary dark:hover:text-accent transition-colors">Services</a></li>
              <li><a href="/profile" onClick={handleNavigation("/profile")} className="font-medium hover:text-primary dark:hover:text-accent transition-colors">Profile</a></li>
              <li><a href="/contact" onClick={handleNavigation("/contact")} className="font-medium hover:text-primary dark:hover:text-accent transition-colors">Contact</a></li>
            </ul>
          </nav>

          {/* Right Side */}
          <div className="flex items-center space-x-2 sm:space-x-4">
            <button className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none" onClick={toggleSearch} aria-label="Search">
              <i className="fas fa-search text-gray-600 dark:text-gray-300"></i>
            </button>
            <button className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none" onClick={toggleTheme} aria-label="Toggle dark mode">
            ...
            <button type="button" className="md:hidden inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none" onClick={toggleMobileMenu} aria-expanded={isMobileMenuOpen}>
              ...
              <div className="px-3 py-2">
                ...
                <div className="pl-2 space-y-1 max-h-40 overflow-y-auto">
                  {isLoading ? (
                    ...
                    <span className="block py-1 text-sm text-gray-500">No categories</span>
                  )}
                </div>
              </div>
            </div>
